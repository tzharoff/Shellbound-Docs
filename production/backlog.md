# Backlog

## Prototype

- Build basic third-person controller.
- Test Shell Dash.
- Test camera follow and manual camera control.
- Create movement playground.
- Test slingshot traversal.
- Test junk pickup feel.
- Test Grunk conversion interaction.
- Test Shellvis shop purchase.
- Evaluate Yarn Spinner in Unity for dialogue V1.
- Build `DialogueTrigger`, `DialogueController`, and `DialogueStateStore`.
- Save dialogue flags to local JSON.
- Prototype Shellvis first and repeat dialogue.
- Prototype Grunk first dialogue and weirdness beat.

## Design

- Define first world.
- Define health and failure rules.
- Define slingshot purpose.
- Define first set of cosmetics.
- Define Sunny Shore slice.
- Define Great Red Lobstrocity encounter.
- Define bobblehead role.
- Define completion tracking.

## Narrative

- Write first Shellvis interaction.
- Write first Grunk interaction.
- Write Shellvis repeat interaction.
- Write Grunk weirdness beat.
- Define Shellbert symbol language rules.
- Clarify The Kid's role in the first act.
- Sketch The Great Turtle reveal boundaries.
- Define "Time is funny here" rules.

## UI / UX

- Mock dialogue overlay.
- Mock Shellvis shop.
- Decide HUD needs.
- Decide inventory categories.

## Art / Audio

- Define PS1 style constraints.
- Create player turtle visual target.
- Define Shellvis costume reference.
- Define first movement sound effects.
